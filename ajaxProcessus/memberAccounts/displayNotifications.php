...
if (input::exists("post") && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' && isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] == "https://koreagaming.live/pages/member/accounts/notifications.php") {

    if (token::check(input::get("token"), "display_notifications")) {
        ...
        if ($status == "notification") {
            $filterQuery .= " AND cn.type = ?";
            $parametersQuery[] = "notification";
        } else if ($status == "suspend") {
            $filterQuery .= " AND cn.type = ?";
            $parametersQuery[] = "suspend";
        }
        ...
        foreach ($notificationsDataPartial as $key => $value) {
            ...
            if ($type == "notification") {
                $typeHtml = '<div class="mb-2 mr-2 badge badge-pill badge-success">Notification</div>';
            } else if ($type == "suspend") {
                $typeHtml = '<div class="mb-2 mr-2 badge badge-pill badge-danger" style="color:white !important;">Suspend</div>';
            }
            ...
            $tableBody .= '<td class="text-center">' . $typeHtml . '</td>';
        ...
        $token = token::generate("display_notifications");

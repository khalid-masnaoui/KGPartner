...
if (input::exists("post") && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' && isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] == "https://koreagaming.info/pages/member/member_management/clients_list.php") {
    ...
    if (token::check(input::get("token"), "display")) {
        ...
        $clients = $db->query("SELECT  cl.id,cl.rate, cl.username,cl.name,cl.prefix,cl.register_date, cl.status,cl.spadeEvoSkin, pu.username as parent, IFNULL(cb.balance, '0.00') as balance , GROUP_CONCAT(cp.product_id SEPARATOR ',') as productsIds from clients cl LEFT JOIN clients_balance  cb ON cl.id = cb.client_id LEFT JOIN client_products cp ON cl.id = cp.client_id  JOIN partner_users pu ON cl.pt_id = pu.pt_id $partnerFilter where 1=1 $statusFilterQuery  $nameFilterQuery order by id desc limit $limit offset $offset", $queryParameters)->results();

...
if (input::exists("post") && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' && isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] == "https://koreagaming.info/pages/dashboard.php") {
    ...
    if (token::check(input::get("token"), "display_turnover_chart")) {
        ...
        foreach ($activeProviders as $key => $value) {
            ...
            $data["backgroundColor"] = getTurnoverChartsColor($value);


            $data["data"] = getTurnoverChartsData($labels, $value);

            $dataSets[] = $data;
        }

        $token = token::generate("display_turnover_chart");
...
function getTurnoverChartsColor($key)
{
    switch ($key) {
        //casinos
        case 'evo':
...
//get turnover data
function getTurnoverChartsData($labels, $key)
{
    global $db;
    global $period;
    global $prefixes;
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        foreach ($prefixes as $key2 => $value2) {
            ...
            $sql = "SELECT sum(amount) as amountTotal from {$debitTableName} where timestamp >= ? AND timestamp <= ?";
            $parametersQuery = [$periodOneParameter, $periodTwoParameter];

            $amountTotal = $db->query($sql, $parametersQuery)->first()["amountTotal"];

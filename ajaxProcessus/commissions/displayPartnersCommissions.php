...
if (input::exists("post") && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' && isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER'] == "https://koreagaming.live/pages/infos/commissions/partners_commissions.php") {

    if (token::check(input::get("token"), "display_commissions")) {
        ...
        if ($partner != 'all') {
            $filterQuery .= " AND pc.partner_id = ?";
            $parametersQuery[] = $partner;
        }

        $filterQuery .= " AND (pc.created_at >= ? AND pc.created_at <= ?) ";
        $parametersQuery[] = $from;
        $parametersQuery[] = $to;


        $sql = "SELECT cl.name, pu.username , pc.amount, pc.created_at FROM partners_commissions pc JOIN clients cl ON pc.client_id = cl.id JOIN partner_users pu ON pc.partner_id = pu.id WHERE 1=1 {$filterQuery} ORDER BY pc.created_at desc LIMIT $offset, $limit";
        ...
        foreach ($CommissionsData as $key => $value) {
            ...
            $tableBody .= '<td class="text-center" style="font-weight: bold;">' . escape($value["amount"]) . ' 원</td>';
        ...
        $sql = "SELECT count(*) as rowsCount FROM partners_commissions pc  JOIN partner_users pu ON pc.partner_id = pu.id  WHERE 1=1 {$filterQuery}";
        ...
        if ($tableBody == '') {
            $tableBody = '<tr> <td class="text-center" colspan="5">No Data Available!</td> </tr>';
        } else {
            ...
            //add total
            $sql = "SELECT sum(amount) as totalAmount FROM partners_commissions pc JOIN partner_users pu ON pc.partner_id = pu.id   WHERE 1=1 {$filterQuery}";
        ...
        $token = token::generate("display_commissions");
